...
import motherlode.base.api.resource.builder.adapter.arrp.dummy.ArrpRuntimeResourcePack;
...
public record ArrpDataPackBuilderAdapter(ArrpRuntimeResourcePack pack) implements DataPackBuilder {
    ...
    public DataPackBuilder addLootTable(Identifier id, Processor<LootTableBuilder> f) {
        var typeAdapter = new ArrpLootTableBuilderTypeAdapter();
        f.accept(typeAdapter);

        ArrpJLootTable table = ArrpJLootTable.loot(typeAdapter.getType().toString());
